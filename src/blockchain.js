...
class Blockchain {
  ...
  getBlockByHash(hash) {
    let self = this;
    return new Promise((resolve, reject) => {
      let getHash = self.chain.filter((block) => block.hash === hash)[0];
      if (getHash) {
        resolve(getHash);
      } else {
  ...
  validateChain() {
    ...
    return new Promise(async (resolve, reject) => {
      self.chain.forEach(async (block) => {
        let data = await block.validate();
        if (!data) {
          errorLog.push({ error: "Not validated" });
        } else if (
          block.height != 0 &&
          block.previousBlockHash !== self.chain[block.height - 1].hash
        ) {
          errorLog.push(new Error(block + " isn't validated"));
        }
      });
      resolve(errorLog);
    });
